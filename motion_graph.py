...
def game_simulator(file, half, start, stop):
    ...
    df['jersey_number'] = df['jersey_number'].astype('str').replace('\.0', '', regex=True)
    ...
    color_discrete_map = {'Wydad': colour0, 'Adversaire': colour1, 'Ball': colour_ball}

    # Plotly Express version
    fig = px.scatter(df, x="x", y="y", color="Team", hover_name="jersey_number", animation_frame="time",
                     animation_group="jersey_number", range_x=[-0.05,1.05], range_y=[-0.05,1.05], size='size', size_max=10,
                     opacity=0.8, color_discrete_map=color_discrete_map, text='jersey_number',
                     hover_data={'x': False,'y':False,'time':False,'size':False, 'Team':False, 'jersey_number':False})

    # Make jersey number really small inside markers
    fig.update_traces(textfont_size=7, textfont_color='white', hovertemplate=None, hoverinfo='none')
    fig.update_yaxes(autorange="reversed")

    fig.update_layout(xaxis=dict(range=[-0.05,1.05]), yaxis=dict(range=[-0.05,1.05]), coloraxis_showscale=False)
    ...
    fig.update_traces(marker=dict(size=10, opacity=.9),
                      selector=dict(mode='markers'))

    fig.update_layout(margin=dict(l=20, r=20, b=10, t=20))

    fig.update_layout(legend_orientation='v', transition={'duration': 0,'ordering':'traces first'})
    ...
    if export == 'y':
        export_file_name = input('Please enter a name for the json file to be exported (ending with .json): ')
...
app.layout = html.Div([
    dcc.Graph(figure=game_simulator(filename, int(half), int(start_time), int(end_time)))
])
